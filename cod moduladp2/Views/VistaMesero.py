...
class MeseroVista:
    ...
    def hotkeys(self):
        try:
            ...
            self.ventana.bind_all("<Left>", lambda event: self.eventMostrarPanelCliente(event))
            ToolTip(self.btnClientes, "Mostrar clientes Left ")

            # Hotkey para mostrar el panel de comandas (Ctrl + O)
            self.ventana.bind_all("<CRight>", lambda event: self.eventMostrarPanelComanda(event))
            ToolTip(self.btnComandas, "Mostrar comandas Right ")
    ...
    def __init__(self, ventana, sistema):
        try:
            ...
            # Cargar íconos
            try:
                self.iconCliente = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\audiencia.png")
                self.iconComandas = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\ordenar-comida (1).png")
                self.iconGuardar = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\disk.png")
                self.iconCancelar = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\cancelar.png")
                self.iconMesa = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\cena.png")
                self.iconEstado = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\cambio.png")
                self.iconAyuda = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\informacion.png")
                self.iconComandaN = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\ordenar-comida.png")
                self.iconAgregarUsuario = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\user_add.png")
                self.iconEliminarUsuario = PhotoImage(file=r"proyecto-final-NODO-master\cod moduladp2\src\icons\user_delete.png")
