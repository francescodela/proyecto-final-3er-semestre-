...
#COMENTADO PORQUE YA HAY OTRA FUNCION MEJORADA EN CONTROLLERS 18/06
'''
    def cargar_usuario_actual(self, cedula):
        query = "SELECT cedula, nombre, telefono, direccion, email FROM administrador WHERE cedula = ?"
        self.db.hacerConsultas(query, (cedula,))
        datos = self.db.cursor.fetchone()
        if datos:
            self.usuario_actual = Administrador(*datos)
        else:
         ...
         '''   
'''
    def generar_informe_usuario_actual(self, fecha=None):
        if self.usuario_actual and isinstance(self.usuario_actual, Administrador):
            return self.usuario_actual.generar_informe(self.db, fecha)
        else:
            print("Usuario actual no es administrador o no está cargado")
            return None
'''
class Mesa:
